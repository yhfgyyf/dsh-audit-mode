...
async function inspectStoredSession (persistence, sessionId) {
  if (typeof persistence.open !== 'function') return await persistence.inspect(sessionId)
...
export class AuditService extends Service {
  ...
  async requestNow (sessionId, { final = false } = {}) {
    return await this._withLock(sessionId, async () => {
      ...
      if (events === undefined) events = (await inspectStoredSession(this.ctx.sessionPersistence, sessionId)).events
  ...
  async reconcile () {
    for (const sessionId of await this.store.list()) {
      try {
        await inspectStoredSession(this.ctx.sessionPersistence, sessionId)
